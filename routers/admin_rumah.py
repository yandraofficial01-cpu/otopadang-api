...
from models import House # <--- GANTI KE House
from.admin_auth import require_admin # <--- kasih titik biar aman
...
def get_all_rumah(db: Session = Depends(get_db), admin = Depends(require_admin)):
    rumah_list = db.query(House).order_by(House.id.desc()).all() # <--- GANTI
...
def create_rumah(data: RumahCreate, db: Session = Depends(get_db), admin = Depends(require_admin)):
    new_rumah = House(**data.dict()) # <--- GANTI
    db.add(new_rumah)
    db.commit()
...
def update_rumah(rumah_id: int, data: RumahUpdate, db: Session = Depends(get_db), admin = Depends(require_admin)):
    rumah = db.query(House).filter(House.id == rumah_id).first() # <--- GANTI
    if not rumah:
