...
from models import Car, User # TAMBAH USER
...
# GANTI PATH JADI /all-public BIAR GAK TABRAKAN
...
# 3. DETAIL MOBIL - UDAH BIKIN PUBLIC
@router.get("/{car_id}", response_model=CarResponse)
def get_car_detail(
    car_id: int,
    db: Session = Depends(get_db)
    # HAPUS get_current_user. BIAR PUBLIC BISA BUKA
):
...
async def create_car(car_data: CarCreate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    new_car = Car(showroom_id = user_showroom_id if role == "showroom" else car_data.showroom_id, status='pending', **car_dict)
    db.add(new_car)
    db.commit()
    db.refresh(new_car)
    return new_car
